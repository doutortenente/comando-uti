// ============================================================================
// SASI · LeitoCard — Gemini-style + Ondas 2, 5
// Onda 5: dispositivos chips, isolamento badge, severidade_visual border.
// Onda 3: SmartPasteBox expansível na parte inferior.
// Root alterado de <button> para <div> pra suportar elementos interativos aninhados.
// ============================================================================
import {
  Activity, AlertTriangle, Clock, Droplets, Flame, Heart,
  Skull, TrendingDown, TrendingUp, Wind, Pill,
} from 'lucide-react';
import type { DashboardRow, Paciente } from '../lib/supabaseClient';
import { sofaColorClass } from '../lib/drugs';
import { ISOLATION_TYPES } from '../lib/clinical-config';
import DispositivosChips from './clinical/DispositivosChips';
...
interface Props {
  ...
  compact?: boolean;
  onUpdatePatient?: (id: string, data: Partial<Paciente>) => Promise<void>;
}
...
function isolationBadge(isolation: DashboardRow['isolation']) {
  if (!isolation || isolation === 'none') return null;
  const spec = ISOLATION_TYPES.find((t) => t.id === isolation);
  if (!spec || !spec.badgeColor) return null;
  return (
    <span className={`absolute top-2 left-2 text-[9px] font-black uppercase px-1.5 py-0.5 rounded border ${spec.badgeColor}`}>
      {spec.label}
    </span>
  );
}

export default function LeitoCard({ row, onSelect, compact = false, onUpdatePatient }: Props) {
  ...
  // Onda 5: borda semafórica
  const sevClass = row.severidade_visual ? `sev-${row.severidade_visual}` : '';

  // Onda 5: dispositivos normalizados
  const dispositivos = (row.dispositivos ?? {}) as Paciente['dispositivos'] & object;

  function handleDispositivosUpdate(patch: NonNullable<Paciente['dispositivos']>) {
    onUpdatePatient?.(row.paciente_id, { dispositivos: patch });
  }

  return (
    <div
      className={`group relative text-left w-full rounded-2xl border border-r border-y transition shadow-md sasi-fade-in card-grav-${row.gravidade} ${sevClass} ${
        compact ? 'p-3' : 'p-4'
      } ${isSeptic ? 'sasi-critical-pulse' : row.gravidade === 'critico' ? 'sasi-critical-pulse' : ''} hover:shadow-lg hover:-translate-y-1`}
    >
      {/* Onda 5: Isolation badge (top-left absolute) */}
      {isolationBadge(row.isolation)}

      {/* Clickable overlay — cobre o card exceto as chips e paste box */}
      <div
        className="absolute inset-0 cursor-pointer rounded-2xl"
        onClick={() => onSelect?.(row.paciente_id)}
        onKeyDown={(e) => {
          if (e.key === 'Enter' || e.key === ' ') onSelect?.(row.paciente_id);
        }}
        role="button"
        tabIndex={0}
        aria-label={`Abrir paciente ${row.nome}, leito ${row.leito}`}
      />

      {/* Relative z-10 wrapper so interactive children sit above the overlay */}
      <div className="relative z-10">
        {/* HEADER — Leito + UTI + SOFA badge */}
        <div className={`flex items-baseline justify-between gap-2 ${compact ? 'mb-0.5' : 'mb-1'} ${row.isolation && row.isolation !== 'none' ? 'pl-16' : ''}`}>
        ...
        {/* HD compact: 1 linha | normal: 2 linhas */}
        {compact ? (
        ...
        {/* Onda 5 — Dispositivos Chips (non-compact only para evitar overflow) */}
        {!compact && onUpdatePatient && (
          <div className="mt-2 pt-2 border-t border-app-border/30">
            <DispositivosChips
              dispositivos={dispositivos ?? {}}
              onUpdate={handleDispositivosUpdate}
            />
          </div>
        )}

        {/* Onda 3 — SmartPasteBox (non-compact) */}
        {!compact && (
          <SmartPasteBox row={row} />
        )}
      </div>
    </div>
  );
