...
import { Bell, BedDouble, ClipboardCopy, FileDown, LayoutGrid, LayoutList, Plus, RefreshCw, ShieldCheck, Users } from 'lucide-react';
import { LeitoCardSkeleton, SplitSkeleton, EmptyState } from './Skeletons';
import NovoLeitoModal from './NovoLeitoModal';
import UTITabsHeader, { type ActiveUTI } from './UTITabsHeader';
...
function loadActiveUTI(): ActiveUTI {
  const v = localStorage.getItem('uti_active');
  if (v === 'UTI2' || v === 'UTI3' || v === 'UTI4') return v;
  return 'UTI2';
}

export default function Dashboard({ session }: Props) {
  const { dashboard, loading, error, updatePatient } = useSupabasePatients();
  const { totalCriticos, totalWarnings } = useClinicalAlerts();
  const { addToast } = useToasts();
  const { viewMode, setViewMode, cycleTheme } = useUI();
  const [activeUTI, setActiveUTI] = useState<ActiveUTI>(loadActiveUTI);
  const [isCompactView, setIsCompactView] = useState<boolean>(
    () => localStorage.getItem('uti_compact_view') === 'true',
  );
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [showNovoLeito, setShowNovoLeito] = useState(false);
  const [showShortcuts, setShowShortcuts] = useState(false);

  // Persist Onda 4 + Onda 2 states
  useEffect(() => { localStorage.setItem('uti_active', activeUTI); }, [activeUTI]);
  useEffect(() => { localStorage.setItem('uti_compact_view', String(isCompactView)); }, [isCompactView]);
  ...
  const visible = useMemo(
    () => dashboard.filter((r) => r.uti === activeUTI),
    [dashboard, activeUTI],
  );
    ...
    <div className="min-h-screen bg-app text-app-text">
      ...
      <header className="sticky top-0 z-20 bg-app-card/95 backdrop-blur border-b border-app-border">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <UTITabsHeader
              dashboard={dashboard}
              activeUTI={activeUTI}
              onSelect={setActiveUTI}
            />

            <div className="w-px h-6 bg-app-border/60 mx-1" aria-hidden="true" />
            ...
            {dashboard.length > 0 && (
              <>
                <button
                  onClick={async () => {
                    const texto = gerarTextoPlantao(visible, activeUTI);
            ...
            <button
              ...
              className={`p-2 rounded transition ${
                isCompactView
                  ...
                  : 'hover:bg-app-tertiary text-app-text-muted hover:text-app-text'
              }`}
              title={isCompactView ? 'Vista expandida (C)' : 'Vista compacta (C)'}
            >
        ...
        <div className="max-w-7xl mx-auto px-4 pb-3 flex flex-wrap items-center gap-2">
          ...
          {isCompactView && (
            <span className="ml-auto text-[10px] text-app-text-muted/70 italic">
              vista compacta · tecla C pra expandir
            </span>
          )}
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 py-4">
        ...
        {loading && viewMode === 'plantao' && (
          <div className={`grid gap-3 ${isCompactView ? 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'}`}>
        ...
        {!loading && visible.length === 0 && (
          <EmptyState
            icon={BedDouble}
            title={`Nenhum leito ativo em ${activeUTI}`}
        ...
        {!loading && visible.length > 0 && (
          <>
            {viewMode === 'plantao' && (
              <div className={`grid gap-3 ${isCompactView ? 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'}`}>
                {visible.map((row) => (
                  <LeitoCard key={row.paciente_id} row={row} onSelect={setSelectedId} compact={isCompactView} onUpdatePatient={updatePatient} />
                ))}
              </div>
            )}
            {viewMode === 'round' && (
